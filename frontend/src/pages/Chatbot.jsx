...
const SUGGESTIONS = {
  auto:   ["Show all employees", "Show me company policy?", "Who joined this month?", "List departments"],
...
function Message({ role, text, sources, isTyping, tool }) {
  ...
  return (
    <div className={`flex gap-2 sm:gap-3 ${isUser ? "flex-row-reverse" : "flex-row"}`}>
      <div className={`w-7 h-7 sm:w-8 sm:h-8 rounded-xl flex-shrink-0 flex items-center justify-center text-white shadow-sm
        ${isUser ? "bg-slate-700" : "bg-blue-600"}`}>
        {isUser ? <UserIcon /> : <BotIcon />}
      </div>

      <div className={`flex flex-col gap-1 min-w-0 ${isUser ? "items-end max-w-[85%] sm:max-w-[80%]" : "items-start max-w-[85%] sm:max-w-[80%]"}`}>
        {isUser && toolInfo && toolInfo.id !== "auto" && (
          <span className="text-xs text-slate-400">{toolInfo.icon} {toolInfo.label}</span>
        )}

        <div
          className={`px-3 py-2.5 sm:px-4 sm:py-3 rounded-2xl text-sm sm:text-[15px] leading-relaxed prose prose-slate max-w-none w-full
            ${isUser
              ? "bg-slate-800 text-white rounded-tr-none shadow-md prose-invert"
...
function ToolSelector({ selected, onChange }) {
  return (
    ...
    <div className="flex items-center gap-1.5 overflow-x-auto scrollbar-none flex-nowrap sm:flex-wrap pb-0.5">
      {TOOLS.map(tool => {
...
function ConvPanel({ convs, activeThread, onSelect, onNew, onDelete, onRename, onClose }) {
  ...
  return (
    <>
      ...
      {/* Panel — full-height slide-over on mobile, sidebar on desktop */}
      <div className="
        fixed right-0 top-0 bottom-0 z-30 w-[min(320px,90vw)]
        sm:relative sm:w-80 sm:z-auto sm:flex-shrink-0
      ...
      ">
        ...
        <div className="flex-shrink-0 flex items-center justify-between px-4 py-3 border-b border-slate-200">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={onClose}
              className="sm:hidden flex items-center justify-center w-8 h-8 rounded-lg border border-slate-200 text-slate-500 hover:bg-slate-50"
            >
              <CloseIcon />
            </button>
          </div>
            ...
            items.length === 0 ? null : (
              <div key={label}>
                ...
                {items.map(conv => (
                  <div
                    key={conv.thread_id}
                    onClick={() => { onSelect(conv.thread_id); onClose(); }}
                    className={`
  ...
  return (
    <div className="flex h-full min-h-0 w-full overflow-hidden bg-slate-50">
      ...
      <div className="flex min-h-0 flex-1 min-w-0 flex-col">
        ...
        {/* Header */}
        <div className="flex-shrink-0 bg-white border-b border-slate-200 px-3 sm:px-4 py-2 sm:py-2.5 flex items-center justify-between gap-2">
          <div className="flex items-center gap-2 sm:gap-3 min-w-0">
            <div className="w-7 h-7 sm:w-8 sm:h-8 bg-blue-600 rounded-xl flex-shrink-0 flex items-center justify-center text-white shadow-sm">
              <BotIcon />
            </div>
            <div className="min-w-0">
              <p className="text-sm sm:text-base font-semibold text-slate-900 truncate">{config.assistant_name}</p>
              <p className="text-xs text-slate-500 truncate">{config.client_name} · AI Assistant</p>
            </div>
          </div>

          <div className="flex items-center gap-2 flex-shrink-0">
            {/* Online badge — hide label on very small screens */}
            <span className="flex items-center gap-1.5 text-xs text-emerald-700 bg-emerald-50 px-2 sm:px-3 py-1 rounded-full font-medium">
            ...
            <button
              ...
              className="flex items-center gap-1.5 text-sm text-slate-600 hover:text-slate-900
                border border-slate-200 hover:border-slate-300 px-2 sm:px-3 py-1.5 rounded-lg
                bg-white hover:bg-slate-50 transition-all shadow-sm"
            >
              <PanelIcon />
              <span className="hidden sm:inline">{panelOpen ? "Hide" : "Chats"}</span>
            </button>
          </div>
        </div>
        ...
        <div className="flex-1 min-h-0 overflow-y-auto px-3 sm:px-4 py-4">
          <div className="mx-auto flex min-h-full w-full max-w-4xl flex-col">
            ...
            {messages.length === 0 && !loading && (
              <div className="flex flex-1 flex-col items-center justify-center py-8 sm:py-16 text-center">
                <div className="w-14 h-14 sm:w-16 sm:h-16 bg-blue-50 rounded-2xl flex items-center justify-center text-blue-600 mb-4 text-2xl sm:text-3xl shadow-sm">
                  {currentTool?.icon || "✨"}
                </div>
                <h2 className="text-lg sm:text-xl font-semibold text-slate-800 mb-2">
                  Hi, I'm {config.assistant_name}
                </h2>
                <p className="text-sm sm:text-base text-slate-500 max-w-sm sm:max-w-md mb-6 sm:mb-8 px-2">
                  {currentTool?.id === "auto"
                    ? "Ask me anything — I'll figure out where to look."
                    : `${currentTool?.label} — ${currentTool?.description}`}
                </p>

                <div className="flex flex-col items-center gap-3 mb-6 sm:mb-8 w-full px-2">
                  <p className="text-xs text-slate-500 font-medium uppercase tracking-wide">Select a tool</p>
                  <ToolSelector selected={activeTool} onChange={setActiveTool} />
                </div>

                {/* Suggestions: 1 col on mobile, 2 on sm+ */}
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3 w-full max-w-sm sm:max-w-lg px-2">
                  {suggestions.map(s => (
                    <button
                      key={s}
                      onClick={() => send(s)}
                      className="text-left text-sm px-4 py-3 bg-white border border-slate-200 rounded-xl
                        ...
                        text-slate-700 transition-all shadow-sm active:scale-[0.98]"
                    >
                      {s}
                    </button>
                  ))}
        ...
        {/* Input area */}
        <div className="flex-shrink-0 bg-white border-t border-slate-200 px-3 sm:px-4 pt-2 pb-3 sm:pb-3">
          <div className="max-w-4xl mx-auto w-full space-y-2 sm:space-y-3">

            {/* Tool selector row */}
            <div className="flex items-center gap-2">
              <span className="text-xs sm:text-sm text-slate-500 flex-shrink-0">Tool:</span>
            ...
            <div className="flex gap-2 sm:gap-3 items-end">
              <div
                className="flex-1 bg-slate-50 border border-slate-200 rounded-2xl
                  ...
                  transition-all flex items-end px-3 sm:px-4 py-2 sm:py-2.5"
              >
                {activeTool !== "auto" && (
                  <span className="text-lg sm:text-xl mr-2 sm:mr-3 flex-shrink-0 mb-0.5">{currentTool?.icon}</span>
                )}
                <textarea
                  ref={inputRef}
                  rows={1}
                  ...
                  placeholder={
                    ...
                    activeTool === "hrms"   ? "Ask about employees, salary..." :
                    activeTool === "policy" ? "Ask about company policies..." :
                    `Ask about ${currentTool?.label}...`
                  }
                  disabled={loading}
                  className="flex-1 bg-transparent text-sm sm:text-base text-slate-900 placeholder:text-slate-400
              ...
              <button
                ...
                className="w-9 h-9 sm:w-10 sm:h-10 bg-blue-600 hover:bg-blue-700 disabled:bg-slate-300
